...
public class LameConfig
{
    ...
    public LibMp3Lame ConfigureDLL(WaveFormat format)
    {
        ...
        {
            if (_preset >= LAMEPreset.V9 && _preset <= LAMEPreset.V0 && result.VBR == LameWrapper.VBRMode.Off &&
                VBR == null) result.VBR = LameWrapper.VBRMode.Default;
        ...
        if (VBR != null)
        {
            result.VBR = (LameWrapper.VBRMode)VBR.Value;
        ...
        if (Mode != null) result.Mode = (LameWrapper.MPEGMode)Mode.Value;
    ...
    /// <summary>
    ///     Select output sampling frequency. If not specified, LAME will automatically resample the input when using high
    ///     compression ratios.
    /// </summary>
    ...
    /// <summary>
    ///     If true then <see cref="VBRMinimumRateKbps" /> is a hard limit, otherwise analog silence will produce lower
    ///     bitrate frames.
    /// </summary>
