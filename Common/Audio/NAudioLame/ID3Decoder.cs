...
#pragma warning restore IDE0059 // Unnecessary assignment of a value
    ...
    public static ID3TagData Decode(byte[] buffer)
    {
        ...
        // decode size field and confirm range
        var size = DecodeHeaderSize(buffer, 6);
    ...
    private static ID3TagData InternalDecode(byte[] buffer, int offset, int size, byte flags)
    {
        ...
        for (var frame = ID3FrameData.ReadFrame(bytes, pos, out var frameSize);
             frameSize > 0 && frame != null;
             frame = ID3FrameData.ReadFrame(bytes, pos, out frameSize))
        {
    ...
    {
        return buffer?.Length >= 4 && buffer[0] == 'I' && buffer[1] == 'D' && buffer[2] == '3' && buffer[3] == 3 &&
               buffer[4] == 0;
    }

    /// <summary>
    ///     Decode a 28-bit integer stored in the low 7 bits of 4 bytes at the offset, most-significant bits first
    ///     (big-endian).
    /// </summary>
    ...
    private class ID3FrameData
    {
        ...
        public static ID3FrameData ReadFrame(byte[] buffer, int offset, out int size)
        {
            ...
            // Extract header data
            var frameID = Encoding.ASCII.GetString(buffer, offset, 4);
            var frameLength = DecodeBEInt32(buffer, offset + 4);
            var frameFlags = DecodeBEInt16(buffer, offset + 8);

            // copy frame content to byte array
            var content = new byte[frameLength];
        ...
        private static string GetUnicodeString(byte[] buffer, ref int offset, bool requireTerminator = true)
        {
            ...
            for (;
                 position < buffer.Length - 1 && (buffer[position] != 0 || buffer[position + 1] != 0);
                 position += 2) ;
            if (requireTerminator && position >= buffer.Length)
                return null;
            ...
            var length = position - start;
            offset = position + 2;
            var res = UCS2.GetString(buffer, start, length);
            return res;
        }
        ...
        public string ParseCommentText()
        {
            ...
            var language = Encoding.ASCII.GetString(Data, position, 3);
            position += 3;
            var shortdesc = getstr(Data, ref position, true);
            var comment = getstr(Data, ref position, false);
        ...
        public KeyValuePair<string, string> ParseUserDefinedText()
        {
            ...
            var position = 1;
            var description = getstring(Data, ref position, true);
            var value = getstring(Data, ref position, false);
        ...
        public APICData ParseAPIC()
        {
            ...
            // get attributes
            var position = 1;
            var mime = getstr(Data, ref position, true);
            var type = Data[position++];
            var description = getstr(Data, ref position, true);

            // get image content
            var datalength = Data.Length - position;
